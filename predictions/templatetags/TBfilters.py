...
@register.filter
def FloatFormatTB(value,d):
    "formats float decimals,  if text it does not change"
...
#            df=df.append({'date_m':date_m, 'match':match, 'player':player ,'odds':odds ,'available-amount':available_amount, 'side':side, 'inplay':inplay},ignore_index=True)
